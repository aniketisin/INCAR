...
fin=open(loc + '../src/tmp/ring_list_' + cnt, 'r')
for line in fin:
	columns = line.split('\t')
...
i=0
fin=open(loc + '../src/scripts/param', 'r')
for line in fin:
	i=i+1
	if (i==5):
		columns = line.split('\t')
...
fout=open(loc + '../src/tmp/parameters_' + cnt, 'w')
...
fout=open(loc + '../src/tmp/stacked_bases_' + cnt, 'w')
fout.write('i\ttype\tring\tchain\tj\ttype\tring\tchain\tdistance\tshift\tslide\trise\troll\ttilt\ttwist\n')
var=-1
count=0
i=1
